...
class NewMemoryViewModel : ViewModel() {
        ...
        if (!noteId.isNullOrEmpty() && _note.value == null) {
            viewModelScope.launch {
                try {
                    ...
                    val fetchedNote =
                        document.toObject(Note::class.java)?.copy(noteId = document.reference.path)
    ...
    fun getPetsForUser(userId: String): StateFlow<List<Pet>> {
        viewModelScope.launch {
            try {
                ...
                val petReferences = userDoc?.get("pets") as? List<DocumentReference> ?: emptyList()
                val pets = petReferences.mapNotNull { petRef ->
                    val petSnap = petRef.get().await()
                    val pet = petSnap.toObject(Pet::class.java)
                    pet?.copy(petId = petSnap.getString("petId") ?: petRef.id)
                }
                _pets.value = pets
            } catch (e: Exception) {
    ...
    fun saveNoteForUser(userId: String, note: Note, onComplete: (Boolean) -> Unit) {
        viewModelScope.launch {
            try {
        ...
        viewModelScope.launch {
            try {
                ...
                FirebaseFirestore.getInstance()
                    ...
                    .update(
                        "usernotes",
                        FieldValue.arrayRemove(FirebaseFirestore.getInstance().document(noteId))
                    )
                    .await()
    ...
    // app/src/main/java/juditgp/com/pawls4eversocial/viewmodel/NewMemoryViewModel.kt
    fun addComment(
        noteId: String,
        ...
        val commentData = mapOf(
            "idComment" to commentId,
            "idNoteRelated" to noteId,
            "idUserProvider" to userIdProvider,
            "content" to content
        )

        val commentDocRef = commentsRef.document(commentId)

        commentDocRef.set(commentData)
            .addOnSuccessListener {
                // Añadir la referencia del comentario al array comments de la nota
                db.collection("notes").document(noteId.split("/").last())
                    .update("comments", FieldValue.arrayUnion(commentDocRef))
                    .addOnSuccessListener {
                        // Notifica al propietario si no es el mismo usuario
                        db.collection("notes").document(noteId.split("/").last()).get()
                            .addOnSuccessListener { noteDoc ->
                                val noteOwnerId =
                                    (noteDoc.get("noteId") as? String)?.split("/")?.lastOrNull()
                                if (noteOwnerId != null && noteOwnerId != userIdProvider) {
                                    val notificationId = notificationsRef.document().id
                                    val notificationData = mapOf(
                                        "idNotification" to notificationId,
                                        "idNoteRelated" to noteId,
                                        "idUserProvider" to userIdProvider,
                                        "idUserReceptor" to noteOwnerId,
                                        "content" to "¡Han comentado tu nota!",
                                        "seen" to false
                                    )
